...
if(@$arguments[1] == 'add' and isset($arguments[2]) and isset($arguments[3])){
    ...
    $this->say('~rule [command] (說話內容) (回話內容)', 'Error');
    $this->say('P.S: (說話內容) 參數中的空格請用 $SP 代替', 'Error');
	$this->say('=================================', 'Error');
	$this->say('指令列表:', 'Error');
	$this->say('列出所有回話規則 [悄悄話方式發送]: ~rule list', 'Error');
    $this->say('列出指定回話規則: ~rule look <說話內容>', 'Error');
	$this->say('新增回話規則: ~rule add <說話內容> <回話內容>', 'Error');
	$this->say('刪除回話規則 [管理指令]: ~rule delete <說話內容>', 'Error');
	$this->say('=================================', 'Error');
}